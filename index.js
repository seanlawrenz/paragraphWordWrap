const text = `WASHINGTON—Unable to rest their eyes on a colorful photograph or boldface heading that could be easily skimmed and forgotten about, Americans collectively recoiled Monday when confronted with a solid block of uninterrupted text.
...
const splitterReducer = arr =>
  arr.map(paragraph => {
    // Remove the new lines on the string
    paragraph = paragraph.replace(/\r?\n|\r/g, ' ');
    const paragraphCutToMaxCharAllowed = [];
    while (paragraph.length > 80) {
      // Get the number of characters allowed before 80 or last word.
      let numberOfCharsAllowed = paragraph.substring(0, 80).lastIndexOf(' ');
      numberOfCharsAllowed = numberOfCharsAllowed <= 0 ? 80 : numberOfCharsAllowed;
      paragraphCutToMaxCharAllowed.push(paragraph.substring(0, numberOfCharsAllowed));
      let i = paragraph.indexOf(' ', numberOfCharsAllowed) + 1;

      // Cutting off the part of the string that was pushed
      if (i < numberOfCharsAllowed || i > numberOfCharsAllowed + 80) {
        i = numberOfCharsAllowed;
      }
      paragraph = paragraph.substring(i);
    }
    paragraphCutToMaxCharAllowed.push(paragraph);
    paragraphCutToMaxCharAllowed.push('');
    return paragraphCutToMaxCharAllowed;
  });

// Finally currying the functions to print the new format
const printWithLimitedWidth = text =>
  splitterReducer(splitter(text))
    .flat()
    .map(line => console.log(line));

printWithLimitedWidth(text);
